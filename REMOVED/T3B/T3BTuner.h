...
{
  public:
    T3BTuner(HardwareSerial& stream, uint8_t const resetPin);
    T3BTuner(HardwareSerial& stream, uint8_t const resetPin, uint8_t const spiCsPin);
    void init();
    ...
    bool memorySet(StreamMode const mode, StreamId const id, uint32_t const programId);
    bool memoryGet(StreamMode const mode, StreamId const id, uint32_t* const programId);
