...
static const uint32_t TunerSerialBaudrate = 115200U;//57600U;
...
T3BTuner::T3BTuner(HardwareSerial& stream, uint8_t const resetPin) :
    T3BTuner(stream, resetPin, UnusedPin)
{
}
 ...
 *   Set preset
 */
bool T3BTuner::memorySet(StreamMode const mode, StreamId const id, uint32_t const programId)
{
 ...
 *  Get preset
 */
bool T3BTuner::memoryGet(StreamMode const mode, StreamId const id, uint32_t* const programId)
{
...
bool T3BTuner::responseReceive()
{
    ...
    while (millis() < endMillis && index < T3BTunerMaxDataSize)
    {
        if (stream.available())
        {
            data = stream.read();
